...
class RobotController:
    """Robot arm controller (7-DOF)"""
    def __init__(self, arm_port=5555):
    ...
    def init(self):
        try:
            ...
            print("[INFO] Robot arm controller initialized successfully")
...
def send_request(image_array: np.ndarray, instruction: str, server_url: str):
    ...
    if not isinstance(action, list) or len(action) != 7:
        print(type(action))
        raise ValueError(f"Expected 7-dimensional action, got: {action}")
...
def run_closed_loop_control():
    ...
    # Initialize robot
    robot_controller = RobotController(arm_port)
