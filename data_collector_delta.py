...
class D1DataRecorder:
    def __init__(self):
        ...
        self.d1_command_deltas = []  # Store deltas for joints 1-6
        self.d1_command_positions = []  # Store positions for accessing joint 7
    ...
    def _calculate_deltas(self, positions_array):
        """Calculate delta positions from absolute positions array"""
        ...
        for i in range(len(positions_array)):
            if i == 0:
                # First position - use zeros as delta
                deltas.append([0.0] * len(positions_array[i]))
            else:
                # Calculate delta from previous position
                delta = [positions_array[i][j] - positions_array[i-1][j] 
                        for j in range(len(positions_array[i]))]
                deltas.append(delta)
    ...
    def SynchronizeData(self):
        ...
        # Synchronize D1 command data and calculate deltas
        if len(self.d1_command_timestamps_raw) > 1 and len(self.d1_command_positions_raw) > 0:
            print("Synchronizing D1 command data and calculating deltas...")
            d1_cmd_timestamps = np.array(self.d1_command_timestamps_raw)
            d1_cmd_positions = np.array(self.d1_command_positions_raw)
            
            # First interpolate to get synchronized positions
            sync_positions = self._interpolate_array(d1_cmd_timestamps, d1_cmd_positions, self.sync_timestamps)
            
            # Store synchronized positions for joint 7 access
            self.d1_command_positions = sync_positions
            
            # Then calculate deltas
            self.d1_command_deltas = self._calculate_deltas(sync_positions)
    ...
    def SaveToJSON(self):
        ...
        for step_id in tqdm(range(num_samples), desc="Processing frames"):
            ...
            # Add D1 delta positions for first 6 joints and gripper state for joint 7
            if step_id < len(self.d1_command_deltas) and len(self.d1_command_deltas[step_id]) > 0:
                # First 6 joints as deltas
                d1_deltas = self.d1_command_deltas[step_id][:6]
                action.extend(d1_deltas)
                
                # Joint 7 as gripper state (0 or 1) based on absolute position
                if step_id < len(self.d1_command_positions) and len(self.d1_command_positions[step_id]) > 6:
                    joint7_position = self.d1_command_positions[step_id][6]
                    gripper_state = 1.0 if joint7_position <= -10 else 0.0
                else:
                    gripper_state = 0.0
            ...
            entry = {
                "image": os.path.join("./data/robotdog/trash_bussing_0", f"{self.current_episode_id:06d}", image_filename),
        ...
        print(f"  - Action format: [d1_joint1_delta, ..., d1_joint6_delta, gripper_state(0/1), base_x_vel, base_y_vel, base_yaw_vel]")
